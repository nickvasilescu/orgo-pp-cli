...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".orgo-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ORGO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ORGO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/nickvasilescu/orgo-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.orgo-pp-cli/feedback.jsonl.
When ` + "`ORGO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ORGO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  orgo-pp-cli feedback list
  orgo-pp-cli feedback list --limit 5
  orgo-pp-cli feedback list --json`,
