# Orgo Printed CLI Agent Guide

This directory is a generated `orgo-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
orgo-pp-cli doctor --json
orgo-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
orgo-pp-cli which "<capability>" --json
orgo-pp-cli <command> --help
```
...
```bash
orgo-pp-cli <command> --help
orgo-pp-cli <command> --dry-run --agent
```
